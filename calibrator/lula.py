...
class LULA(BaseCalibrator):
    ...
    def post_calibrate(self, optimizer: torch.optim.Optimizer) -> None:
        
        if isinstance(self.model, BaseCalibrator):
            self.model.post_calibrate(optimizer)        
        
        var0 = torch.tensor(1/(5e-4*self.num_train)).float().to(self.device)
        nll  = torch.nn.CrossEntropyLoss(reduction='mean')
        n_units_grid = [[4], [8], [16], [32]]
        best_loss_noise = float('inf')
        
        best_network_statedict = self.network.state_dict()
        
        self.network.eval()
        for n_unit in n_units_grid:
        
            n_lula_units = n_unit
            network = LULAModel(self.network, n_lula_units, device=self.device).to(self.device)
            network.enable_grad_mask()
            
            self.calibrate_loader.dataset.offset = np.random.randint(len(self.calibrate_loader.dataset))
            network = train_lula_layer(
                network, nll, self.valid_loader, self.calibrate_loader, 1/var0, 
                lr=self.lr, n_iter=1, progressbar=False, mc_samples=10, device=self.device
            )
            
            network.disable_grad_mask()
            network.eval()
            network.unmask()
            network.base_model.linear.weight.requires_grad = False
            
            curr_model_statedict = network.state_dict()
            
            # Do a LA over the trained LULA-augmented network
            model_kfla = KFLA(network, device=self.device).to(self.device)
            model_kfla.get_hessian(self.train_loader)
            model_kfla.estimate_variance(var0)
            out_in  = kfla_predict(self.valid_loader, model_kfla, n_samples=10, device=self.device)
            out_out = kfla_predict(self.calibrate_loader, model_kfla, n_samples=10, n_data=2000, device=self.device)
            ...
            if loss < best_loss_noise:
                best_network_statedict = curr_model_statedict
...
class MaskedLinear(torch.nn.Module):
    ...
    def to_gpu(self):
        self.mask_w = self.mask_w.to(self.device)
        self.mask_b = self.mask_b.to(self.device)

    def to_unmasked(self):
        lin = torch.nn.Linear(self.n_in+self.m_in, self.n_out+self.m_out)
...
class MaskedConv2d(torch.nn.Module):
    ...
    def __init__(self, base_layer, m_in, m_out, device=None):
        ...
        self.mask_w[n_out:, :, :, :] = 1  # Lower half
        self.mask_w = self.mask_w
        
        self.mask_b = torch.zeros(n_out+m_out)
        self.mask_b[n_out:] = 1
        self.mask_b = self.mask_b
        self.switch_grad_mask(True)
    ...
    def switch_grad_mask(self, on=True):
        if on:
            self.grad_handle_w = self.weight.register_hook(lambda grad: grad.mul_(self.mask_w.to(grad.device)))
            if self.bias:
                self.grad_handle_b = self.bias.register_hook(lambda grad: grad.mul_(self.mask_b.to(grad.device)))
        else:
            self.grad_handle_w.remove()
    ...
    def to_unmasked(self):
        conv = torch.nn.Conv2d(self.n_in+self.m_in, self.n_out+self.m_out, self.kernel_size, self.stride, self.padding, self.dilation, self.groups)
...
# >>> Model 
class LULA_BasicBlock(torch.nn.Module):
    
    def __init__(self, base_model, n_lula_units, device=None):
        """
        ...
        base_modules = [m for m in base_model.modules() if type(m) != torch.nn.Sequential and type(m) != type(base_model)]

        # Zero unit for both input and output layers
        n_lula_units = [0] + n_lula_units + [0]
        assert len(n_lula_units) == 1 + len([m for m in base_modules if type(m) == torch.nn.Linear or type(m) == torch.nn.Conv2d])
        ...
        self.lula_conv2 = modules[2]
        self.bn2  = torch.nn.BatchNorm2d(self.lula_conv2.out_channels)
        self.shortcut = base_model.shortcut
...
class LULAModel(torch.nn.Module):
    ...
    def __init__(self, base_model, n_lula_units, device=None):
        ...
        self.lula_layer = LULA_BasicBlock(base_model.layer4[1], n_lula_units, device=self.device)
        self.base_model.layer4[1] = self.lula_layer

    def forward(self, x):
        return self.base_model(x)

    def features(self, x):
        return self.base_model.partial_forward(x)
...
def train_lula_layer(lula_model, nll, in_loader, out_loader, prior_prec, l2_penalty=0, lr=1e-1, n_iter=1,
    ...
    opt = torch.optim.Adam(filter(lambda p: p.requires_grad, lula_model.parameters()), lr=lr, weight_decay=0)
    ...
    for it in pbar:
        ...
        for (_, x_in, _, _), (x_out, _) in zip(in_loader, out_loader):
            ...
            for s in range(mc_samples):
                ...
                W_conv1_s = mu_W_conv1 + torch.sqrt(sigma_W_conv1) * torch.randn(*mu_W_conv1.shape).to(device)
                ll_module.lula_conv1.weight = torch.nn.Parameter(W_conv1_s)
                if ll_module.lula_conv1.bias:
                    b_conv1_s = mu_b_conv1 + torch.sqrt(sigma_b_conv1) * torch.randn(*mu_b_conv1.shape).to(device)
                    ll_module.lula_conv1.bias = torch.nn.Parameter(b_conv1_s)
                    
                W_conv2_s = mu_W_conv2 + torch.sqrt(sigma_W_conv2) * torch.randn(*mu_W_conv2.shape).to(device)
                ll_module.lula_conv2.weight = torch.nn.Parameter(W_conv2_s)
                if ll_module.lula_conv2.bias:
                    b_conv2_s = mu_b_conv2 + torch.sqrt(sigma_b_conv2) * torch.randn(*mu_b_conv2.shape).to(device)
...
def get_fisher_diag_layer_(model, x, nll, n_samples, lik_prec=1, bias=False):
    ...
    for s in range(n_samples):
        ...
        if type(nll) == torch.nn.BCEWithLogitsLoss:
            y = torch.distributions.Bernoulli(logits=output).sample()
        elif type(nll) == torch.nn.CrossEntropyLoss:
        ...
        grad_conv1 = torch.autograd.grad([loss], p_conv1, retain_graph=True, create_graph=True)[0]
        grad_conv2 = torch.autograd.grad([loss], p_conv2, retain_graph=True, create_graph=True)[0]
...
@torch.no_grad()
def kfla_predict(test_loader, model, n_samples=20, apply_softmax=True, return_targets=False, delta=1, n_data=None, device=None):
    py = []
    targets = []
    count = 0
    ...
    for batch in test_loader:
        ...
        x, y = delta*x.to(device), y.to(device)
        targets.append(y)
...
@torch.no_grad()
def lula_predict(dataloader, model, n_samples=1, T=1, delta=1, return_targets=False, device=None):
    py = []
    targets = []
...
class KFAC(torch.optim.Optimizer):
    ...
    def __init__(self, net, alpha=0.95, device=None):
...
class KFLA(torch.nn.Module):
    ...
    def sample(self, scale=1, require_grad=False):
        for module, name in self.params:
            ...
            if name == 'bias':
                w = module.__getattr__(f'{name}_mean').to(self.device)
            else:
                M = module.__getattr__(f'{name}_mean').to(self.device)
                U_half = module.__getattr__(f'{name}_U_half').to(self.device)
                V_half = module.__getattr__(f'{name}_V_half').to(self.device)
                ...
                E = torch.randn(*M_.shape).to(self.device)
    ...
    def estimate_variance(self, var0, invert=True):
        ...
        for module, name in self.params:
            ...
            if invert:
                ...
                U_ += torch.sqrt(tau)*torch.eye(m).to(self.device)
                V_ += torch.sqrt(tau)*torch.eye(n).to(self.device)

                # For numerical stability
                u = torch.cholesky(U_.cpu() + 1e-6*torch.eye(m))
                v = torch.cholesky(V_.cpu() + 1e-6*torch.eye(n))

                U_ = torch.cholesky(torch.cholesky_inverse(u), upper=False).to(self.device)
                V_ = torch.cholesky(torch.cholesky_inverse(v), upper=True).to(self.device)
    ...
    def get_hessian(self, train_loader, binary=False):
        criterion = torch.nn.BCEWithLogitsLoss() if binary else torch.nn.CrossEntropyLoss()
        opt = KFAC(self.net, device=self.device)
        U = {}
        V = {}
        ...
        for _, (_, x, y, _) in enumerate(train_loader):
    ...
    def gridsearch_var0(self, val_loader, ood_loader, interval, n_classes=10, lam=1):
        ...
        for var0 in pbar:
            try:
                ...
                preds_in, y_in = kfla_predict(val_loader, self, n_samples=5, return_targets=True, device=self.device)
                loss_in = F.nll_loss(torch.log(preds_in + 1e-8), y_in)

                if ood_loader is not None:
                    preds_out = kfla_predict(ood_loader, self, n_samples=5, device=self.device)
...
def kfla_parameters(module, params, device=None):
    mod_class = module.__class__.__name__
    if mod_class not in ['Linear', 'MaskedLinear']:
        return
